...
In terms of proportion of variance explained by the model, the fit is pretty poor. Both $R^2$ and adjusted $R^2$ yields a pretty low value of roughly 51% explained variance. 
...
One observes from figure 2 that the studentized residuals are heteroscedatic, and thus the errors are heteroscedatic. This is problematic, as the linear model assumes homoscedatic errors.
There is also no evidence of any non-linear effects of the covariates looking at the studentized residuals

To look into the significance of the regression one can use the so-called F-test: 
$$
H_0: \beta_j = 0 \;\;\forall j = 0, \dots ,p \quad H_1: \text{at least one} \; \beta_j \ne 0
$$
The F-test yields a low p-value, and the null-hypothesis is rejected. Thus, the regression is significant.
...
Comparing these two model fits by means of $R^2_{\text{adj}}$, one observes that the best model results from the $R^2_{\text{adj}}$-model selection criteria. 

 - compare chosen model with full model -
```{r,eval = TRUE}
